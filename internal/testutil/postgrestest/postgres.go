...
func CleanupTables(t testing.TB, store Store) {
	t.Helper()
	_, err := store.DB().Exec(`
TRUNCATE ban_statuses, chat_messages, chat_invitations, chat_member_permissions, chat_members, chat_room_states, chat_rooms, friends, friend_requests, key_packages, device_push_tokens, user_events, event_subscriptions, auth_sessions, profiles RESTART IDENTITY CASCADE
`)
	if err != nil {
